            ...
            <section class="ready-status">
                ...
                <?php if ($data['collection'] && $data['collection']->start_time): ?>
                    <div class="collection-started">
                        ...
                        // Different views for driver and partner
                        ...
                        <?php else: ?>
                            <a href="<?php echo URLROOT; ?>/drivingpartner/supplierCollection/<?php echo $data['collection']->collection_id; ?>" 
                               class="btn btn-primary">
                                <i class="fas fa-clipboard-list"></i> Manage Supplier Collections
                            </a>
                        <?php endif; ?>
                    </div>
                <?php elseif ($data['isReady']): ?>
                    <p>You are marked as ready for this collection.</p>
                    <?php if ($data['otherTeamMemberReady']): ?>
                        <p class="waiting-message">Waiting for collection to start...</p>
                    <?php else: ?>
                        <p class="waiting-message">Waiting for other team member...</p>
                    <?php endif; ?>
                <?php else: ?>
                    <?php if ($isTimeValid): ?>
                        <?php
                        $baseUrl = RoleHelper::getControllerNameByRole($_SESSION['role_id']);
                        ?>
                        <form action="<?php echo URLROOT; ?>/<?php echo $baseUrl; ?>/setReady/<?php echo $data['schedule']->schedule_id; ?>" method="POST">
