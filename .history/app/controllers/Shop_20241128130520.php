...
class Shop extends Controller {
    ...
    public function updateCart() {
        // Check if it's an AJAX request
        if (!$this->isAjaxRequest()) {
            // If not AJAX, return error response
            $this->jsonResponse([
                'success' => false,
        ...
        try {
            ...
            $success = $this->cartModel->updateQuantity(
                $_SESSION['user_id'],
                $data->product_id,
                $data->quantity
            );
    ...
    public function processPayment() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Success response
            jsonResponse(['status' => 'success', 'message' => 'Payment processed successfully']);
        } else {
            // Error response
            jsonResponse(['status' => 'error', 'message' => 'Invalid request method'], 400);
        }
    }
