...
class M_Order {
    ...
    public function __construct() {
        ...
        // Fix the path to vendor/autoload.php (go up one directory from app)
        $vendorPath = dirname(APPROOT) . '/vendor/autoload.php';
        
        if (defined('STRIPE_SECRET_KEY') && file_exists($vendorPath)) {
            require_once $vendorPath;
            $this->stripeSecretKey = STRIPE_SECRET_KEY;
            \Stripe\Stripe::setApiKey($this->stripeSecretKey);
            $this->stripeEnabled = true;
        } else {
            error_log("Stripe configuration missing or vendor/autoload.php not found at: " . $vendorPath);
        }
    }
    ...
    public function createOrder($userId, $orderData) {
        ...
        try {
            ...
            $this->db->bind(':payment_intent_id', null); // Will be updated after payment intent creation
            ...
            if ($orderData['payment_method'] === 'stripe') {
                $paymentIntent = $this->createPaymentIntent($orderId, $orderData['grand_total']);
                if (!$paymentIntent) {
